...
Phase 3: Parse CSVs        → Extract card data into SQLite (no browser needed)
...
**Option B (faster if the pattern holds):** SportsCardsPro/PriceCharting may serve images from a CDN with predictable URLs based on product ID. If you discover the pattern (check the image `src` on a few card pages), you could skip Phase 4 entirely and construct image URLs directly. Look for patterns like:
```
https://cdn.sportscardspro.com/images/{product_id}.jpg
https://www.pricecharting.com/images/.../{product_id}...
```
...
## After Scraping — Visual Matching

Once you have images, generate CLIP embeddings:
...
This replaces your current LLM-per-image matching workflow:
- **Before:** 30-50 seconds per card (sequential LLM vision calls)
- **After:** 1-3 seconds per card (one embedding + vector search)

## Deploying on Proxmox

Recommended: Ubuntu 24.04 LXC container

```bash
# Allocate resources
# - 2+ CPU cores
# - 4GB RAM (2GB for scraper, extra for Playwright)
# - 100GB disk (images)

# If generating embeddings with GPU, pass through your RTX 3080
# via privileged LXC or use your Komodo container
```

## Files

```
config.py       — URLs, credentials, delays, paths
database.py     — SQLite schema and operations
scraper.py      — Playwright automation (all 5 phases)
embeddings.py   — CLIP embedding generation and search
